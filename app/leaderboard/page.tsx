  ...
  if (error) {
    return (
      <div className="container mx-auto py-12 px-4">
        <div className="text-center text-red-500">Failed to load leaderboard. Please try again later.</div>
      </div>
    )
  }
  ...
  return (
    <div className="container mx-auto py-12 px-4">
      <div className="text-center mb-12">
        ...
        <p className="text-muted-foreground">The most popular excuses of all time</p>
      </div>
...
function renderLeaderboard(excuses: Excuse[], isLoading: boolean, mutate: () => void) {
  if (isLoading) {
    return (
  ...
  if (!excuses || excuses.length === 0) {
    return <div className="text-center py-12 text-muted-foreground">No excuses found for this timeframe.</div>
  }
    ...
    <div className="space-y-6">
      {excuses.map((excuse, index) => (
        <div key={excuse.id} className="relative">
          ...
          <ExcuseCard excuse={excuse} onVoteSuccess={() => mutate()} isLeaderboard rank={index + 1} />
        </div>
      ))}
    </div>
  )
